...
from model_hetero import HHGNN
from sklearn.model_selection import KFold
import os
import glob
import tqdm
from drug_util import GraphDataset, collate
from utils import metrics_graph, set_seed_all, draw_bar
...
def load_data(dataset):
    ...
    for row in synergy:
        if row[3] > 40:
            syn.append([row[0], row[1], row[2], 1])
        elif row[3] < 0:
            syn.append([row[0], row[1], row[2], 0])
    
    drug_sim_matrix, cline_sim_matrix = get_sim_mat(drug_smiles_fea, np.array(gene_data, dtype='float32'))
    return drug_fea, cline_fea, syn, drug_sim_matrix, cline_sim_matrix
...
if __name__ == '__main__':
    ...
    for cv_mode in cv_mode_ls:
        ...
        DTH_cls, DTH_dc, hyperedge, input_weight, num_cls = get_among_cls_h(synergy_data)
        ...
        for train_index, validation_index in kf.split(cv_data):
            ...
            for epoch in tqdm.tqdm(range(epochs)):
                ...
                if val_metric[0] > best_metric[0]:
                    model_path = f'Model_param_cls/{dataset_name}_mode{cv_mode}_fold{fold_num}_{best_epoch}.pth'
            ...
            print('The results on test set, ',
                  'AUC: {:.6f},'.format(test_metric[0]),
                  'AUPR: {:.6f},'.format(test_metric[1]), 
                  'F1: {:.6f},'.format(test_metric[2]),
                  'ACC: {:.6f},'.format(test_metric[3]))
